...
- **Base**: $\ell-1 = 1$. $F_1$ is a set of singletons and cannot contains duplicates by construction. Therefore, taking distinct pairs of itemsets only once, it's not possible to generate duplicates in $C_2$.
- **Induction**: $\ell -1 > 1$. By inductive hypothesis, $C_{\ell-1}$ contains no duplicates. Then, since $F_{\ell-1} \subseteq C_{\ell-1}$, also $F_{\ell-1}$ cannot contain duplicates. Since the only way to generate a duplicate would be to have a duplicate in $F_{\ell-1}$, as argued before, we conclude that $C_\ell$ doesn't contain duplicates. $\square$
...
Consider two association rules:

* $r_1 : A → B$,  and

* $r_2 : B → C$ .

Suppose that both satisfy support and confidence requirements.

Is it true that also $r_3 : A → C$ satisfies the requirements? If so, prove it, otherwise show a counterexample.
...
- $Supp(B \rightarrow C) = Supp(\{BC\})=1/2$
...
Anti monotonicity of support for rules states that for $0 \ne Y' \subset Y \subset Z$ we have:

> $\cfrac{Supp(Z)}{Supp(Z\setminus Y)} \le \cfrac{Supp(Z)}{Supp(Z\setminus Y')}$. 
...
For a given itemset $X = \{x_1 , x_2 , . . . , x_k\}$, define the measure:

>  $\zeta(X ) =\min \{Conf(x_i → X \setminus \{x_i\}) : 1 ≤ i ≤ k\}$. 

Say whether $\zeta$ is monotone, anti-monotone or neither one. Justify your answer.

### Solution $\checkmark$

Consider two itemsets $X$,$Y$ such that $X \subseteq Y$. By definition:

>  $Conf(x_i → X \setminus \{x_i\})=\cfrac{Supp(X)}{Supp(\{x_i\})}$. 

Then, if $x_i$ is the item minimizing the confidence of the rule, by anti monotonicity of support we know that:

>  $\zeta(X) = Conf(\{x_i\}\rightarrow X\setminus \{x_i\})=\cfrac{Supp(X)}{Supp(\{x_i\})} \ge \cfrac{Supp(Y)}{Supp(\{x_i\})}$.

We don't know if $x_i$ is exactly the item minimizing confidence of the rule in $Y$, but since $\zeta$ returns the minimum confidence we know by definition that:

>  $Conf(\{x_i\}\rightarrow Y\setminus \{x_i\}) = \cfrac{Supp(Y)}{Supp(\{x_i\})} \ge \zeta(Y)~ \forall~i$.
...
2. I'll prove that each frequent itemset of length $k$ must be in $C_k$ (i.e. $X \in F_k \implies X \in C_k$ which implies $F_k \subseteq C_k$).
   ...
   1. $Z_1=Z[0]Z[1]\cdots  [k-1]Z[k]Z$ (where $Z_1a=Z$ and $a = Z[k]$)
   
   2. $Z_2=Z[1]Z[2]\cdots Z[k]Z[k+1]$  (where $bZ_2=Z$ and $b=Z[0]$)
...
By induction on $m$. Assume that the algorithm is correct and computes all the rules of length $ < m$ for some value $m$.
...
- **Induction**: by inductive hypothesis $H_{Z, m-1}$ is the correct set of rules towards subsets of length $m-1$.

  Then $APRIORI-GEN$ generates the correct candidates of length $m$. This itemset is refined and only frequent rules are kept.
...
Each itemset that is frequent in a dataset $T$ of $n$ items, must be frequent in some $T_j$ (partition of the dataset, with $1 \leq i \leq k$) each with $O(n/k)$ items.

### Solution

Let $T$ be a dataset of $n$ items, $k$ a number of partitions, and $minsup$ the support threshold. Then each partition contains $O(n/k)$ items.
